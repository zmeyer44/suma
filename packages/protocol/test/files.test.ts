...
describe("cloud-fetch eligibility (§8.6 — authenticated downloads stay local)", () => {
  it("accepts a large public download", () => {
    expect(cloudFetchEligibility(ctx())).toEqual({
      eligible: true,
      reason: "public",
    });
  });
  ...
  it("accepts a presigned URL, whose authorization is already in the link", () => {
    ...
    expect(cloudFetchEligibility(presigned)).toEqual({
      eligible: true,
      reason: "presigned",
    });
    const azure = ctx({
      url: "https://acct.blob.core.windows.net/x/y?sv=2021&sig=deadbeef",
    });
  ...
  it("REFUSES anything carrying a credential — the deleted sealed-request case", () => {
    ...
    for (const over of [
      ...
      expect(verdict.eligible === false && verdict.reason).toBe(
  ...
  it("refuses credentials embedded in the URL itself", () => {
    const verdict = cloudFetchEligibility(
      ctx({ url: "https://user:pw@cdn.example.com/f.bin" }),
    );
    expect(verdict.eligible === false && verdict.reason).toBe(
      "userinfo_in_url",
    );
  ...
  it("refuses private and loopback hosts the cloud machine could not reach anyway", () => {
    ...
    expect(
      cloudFetchEligibility(ctx({ url: "https://[2606:4700::1111]/f.bin" }))
        .eligible,
    ).toBe(true);
  });

  it("keeps small downloads local, and treats unknown size as eligible", () => {
    expect(cloudFetchEligibility(ctx({ totalBytes: 1024 })).eligible).toBe(
      false,
    );
    expect(
      cloudFetchEligibility(ctx({ totalBytes: CLOUD_FETCH_MIN_BYTES - 1 }))
        .eligible,
    ).toBe(false);
    expect(
      cloudFetchEligibility(ctx({ totalBytes: CLOUD_FETCH_MIN_BYTES }))
        .eligible,
    ).toBe(true);
    expect(cloudFetchEligibility(ctx({ totalBytes: null })).eligible).toBe(
      true,
    );
  });
  ...
  it("REFUSES a bearer credential carried in the query string", () => {
    ...
    for (const q of [
      ...
      const v = cloudFetchEligibility(
        ctx({ url: `https://api.example.com/export?${q}` }),
      );
  ...
  it("still accepts genuine object-storage presigned links", () => {
    const s3 =
      "https://b.s3.amazonaws.com/o?X-Amz-Signature=a&X-Amz-Credential=b&X-Amz-Expires=60";
    expect(cloudFetchEligibility(ctx({ url: s3 })).eligible).toBe(true);
    const gcs =
      "https://storage.googleapis.com/b/o?X-Goog-Signature=a&X-Goog-Credential=b";
    ...
    const sas =
      "https://acct.blob.core.windows.net/c/b?sv=2021-08-06&se=2030-01-01&sig=abc";
    expect(cloudFetchEligibility(ctx({ url: sas })).eligible).toBe(true);
    // A bare `sig=` with no SAS scoping is not a presigned link.
    const bare = cloudFetchEligibility(
      ctx({ url: "https://api.example.com/x?sig=abc" }),
    );
...
describe("quota (§8.6 — Pro 100 GB, soft-block)", () => {
  ...
  it("refuses a write that would exceed the limit, without hiding existing data", () => {
    const v = checkQuota(
      { usedBytes: PRO_QUOTA_BYTES - 10, limitBytes: PRO_QUOTA_BYTES },
      100,
    );
  ...
  it("reports soft-blocked once at or over the limit", () => {
    expect(
      checkQuota({ usedBytes: PRO_QUOTA_BYTES, limitBytes: PRO_QUOTA_BYTES }, 1)
        .softBlocked,
    ).toBe(true);
    // Exactly filling the quota is allowed.
    expect(checkQuota({ usedBytes: 0, limitBytes: 100 }, 100).allowed).toBe(
      true,
    );
  });
});
...
describe("vfs paths", () => {
  ...
  it("validates vfs requests", () => {
    expect(
      parseVfsRequest(JSON.stringify({ t: "vfs.list", path: "/" })).t,
    ).toBe("vfs.list");
    expect(() =>
      parseVfsRequest(JSON.stringify({ t: "vfs.nope", path: "/" })),
    ).toThrow();
  ...
  it("parses the new ops, with delete's recursive flag optional", () => {
    expect(
      parseVfsRequest(JSON.stringify({ t: "vfs.tree", path: "/" })).t,
    ).toBe("vfs.tree");
    expect(
      parseVfsRequest(JSON.stringify({ t: "vfs.rename", from: "/a", to: "/b" }))
        .t,
    ).toBe("vfs.rename");
    expect(
      parseVfsRequest(
        JSON.stringify({ t: "vfs.append", path: "/a", dataB64: "" }),
      ).t,
    ).toBe("vfs.append");
    expect(
      parseVfsRequest(
    ...
    const bare = parseVfsRequest(
      JSON.stringify({ t: "vfs.delete", path: "/d" }),
    );
    expect(bare).toEqual({ t: "vfs.delete", path: "/d" });
    const recursive = parseVfsRequest(
      JSON.stringify({ t: "vfs.delete", path: "/d", recursive: true }),
    );
...
describe("vfs responses (wire shapes led by agent/src/vfs.rs)", () => {
  ...
  it("rejects an unknown response tag but accepts unknown error codes", () => {
    ...
    const err = parseVfsResponse(
      JSON.stringify({ t: "error", code: "vfs_new_code", message: "m" }),
    );
