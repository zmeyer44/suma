...
export class TcpAgentClient implements AgentLink {
  ...
  constructor(url: string, options: { capabilityToken?: string } = {}) {
    const { host, port } = parseAgentUrl(url);
    this.host = host;
    this.port = port;
    this.capabilityToken = options.capabilityToken?.trim() || null;
    this.connectCtl();
  }
  ...
  private openChannel(): net.Socket {
    ...
    this.writeAuth(socket);
    socket.on("error", () => socket.destroy());
    return socket;
  }

  private writeAuth(socket: net.Socket): void {
    if (this.capabilityToken !== null) {
      socket.write(encodeFrame("auth", this.capabilityToken));
    }
  }
  ...
  private connectCtl(): void {
    ...
    socket.on("connect", () => {
      ...
      this.writeAuth(socket);
      if (this.capabilityToken === null) this.setUp(true);
    });
    socket.on("data", (chunk) => {
      let frames: AgentFrame[];
      try {
      ...
      for (const frame of frames) {
        if (frame.channel === "auth") {
          if (
            ...
            frame.payload.toString("utf8") === "ok"
          ) {
            this.setUp(true);
          } else {
            socket.destroy();
