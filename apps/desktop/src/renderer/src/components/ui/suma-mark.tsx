/**
 * The Suma mark — a meridian globe, and the same artwork the marketing site's
 * nav carries (`apps/www/src/components/suma-mark.tsx`).
 *
 * COPIED, not imported: the two live in different packages with different
 * build graphs, and a brand mark is a stable asset rather than a moving one.
 * If the artwork is ever redrawn, both files change together — the path data
 * is identical on purpose, so a diff between them is the check.
 *
 * One closed path: the graticule is drawn as HOLES in a solid disc rather than
 * as strokes over it, which is what lets the mark hold its weight at rail size
 * and survive any surface it is set on. It fills with `currentColor`, so the
 * caller's text color is the mark's color.
 */
 ...
 * off-centre next to a wordmark.
 */
export const MARK_BOX = { x: 4, y: 16.2, width: 142.44, height: 117.6 } as const;
